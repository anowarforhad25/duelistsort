...
function App() {
  ...
  const [filter, setFilter] = useState({ February: "", January: "", December: "", Area: "", Balance: "" });
  const [summary, setSummary] = useState({ February: 0, January: 0, December: 0 });
  ...
  const handleLoadData = async () => {
    ...
    try {
      ...
      const final = sheet1.map((row, index) => {
        ...
        const isFebruaryDue = true; // Always assume current month due for this report
        const isJanuaryDue = sheet2Ids.has(customer_id);
        const isDecemberDue = sheet3Ids.has(customer_id);

        const totalCount = (isFebruaryDue ? 1 : 0) + (isJanuaryDue ? 1 : 0) + (isDecemberDue ? 1 : 0);
   ...
   * @param {string} field - The filter field (e.g., "December", "Area").
      ...
      <Box sx={{ backgroundColor: darkTheme.palette.background.default, color: darkTheme.palette.text.primary, minHeight: "100vh" }}>
        ...
        <Box sx={{ px: { xs: 1, sm: 2, md: 3 }, py: 2, minHeight: "calc(100vh - 64px)" }}>
          ...
          <Box display="flex" gap={2} justifyContent="center" flexWrap="wrap" mb={2} sx={{ 
            ...
            {["February", "January", "December", "Area", "Balance"].map((field) => (
              <FormControl key={field} sx={{ minWidth: 120 }} size="small">
                ...
                >
                  ...
                  {(() => {
                    ...
                    if (field === "February" || field === "January" || field === "December") {
