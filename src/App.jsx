...
function App() {
  ...
  const [filter, setFilter] = useState({ March: "", February: "", January: "", Area: "", Balance: "" });
  const [summary, setSummary] = useState({ March: 0, February: 0, January: 0 });
  ...
  const handleLoadData = async () => {
    ...
    try {
      ...
      const final = sheet1.map((row, index) => {
        ...
        const isMarchDue = true; // Always assume current month due for this report
        const isFebruaryDue = sheet2Ids.has(customer_id);
        const isJanuaryDue = sheet3Ids.has(customer_id);

        const totalCount = (isMarchDue ? 1 : 0) + (isFebruaryDue ? 1 : 0) + (isJanuaryDue ? 1 : 0);
   ...
   * @param {string} field - The filter field (e.g., "January", "Area").
      ...
      <Box sx={{ backgroundColor: darkTheme.palette.background.default, color: darkTheme.palette.text.primary, minHeight: "100vh" }}>
        ...
        <Box sx={{ px: { xs: 1, sm: 2, md: 3 }, py: 2, minHeight: "calc(100vh - 64px)" }}>
          ...
          <Box display="flex" gap={2} justifyContent="center" flexWrap="wrap" mb={2} sx={{ 
            ...
            {["March", "February", "January", "Area", "Balance"].map((field) => (
              <FormControl key={field} sx={{ minWidth: 120 }} size="small">
                ...
                >
                  ...
                  {(() => {
                    ...
                    if (field === "March" || field === "February" || field === "January") {
