  ...
  const handleLoadData = async () => {
    ...
    try {
      ...
      const final = sheet1.map((row, index) => {
        ...
        const isOctoberDue = true; // Always assume current month due for this report
        const isSeptemberDue = sheet2Ids.has(customer_id);
        const isAugustDue = sheet3Ids.has(customer_id);

        const totalCount = (isOctoberDue ? 1 : 0) + (isSeptemberDue ? 1 : 0) + (isAugustDue ? 1 : 0);
        ...
        return {
          ...
          August: isAugustDue ? "No Payment" : "Payment",
      ...
      <Box sx={{ backgroundColor: darkTheme.palette.background.default, color: darkTheme.palette.text.primary, minHeight: "100vh" }}>
        ...
        <Box sx={{ px: { xs: 1, sm: 2, md: 3 }, py: 2, minHeight: "calc(100vh - 64px)" }}>
          ...
          <Box mb={4} display="flex" justifyContent="center" gap={{ xs: 2, sm: 4 }} flexWrap="wrap" alignItems="center">
            ...
            <Typography variant="subtitle1" component="span" sx={{ fontSize: { xs: '0.9rem', sm: '1rem' } }}>
              August No Payment: <Box component="span" fontWeight="bold" color="error.main">{summary.August}</Box>
            </Typography>
          </Box>
            ...
            <DialogContent sx={{ maxWidth: { xs: "90vw", sm: "400px" } }}>
              {selectedRow && (
                <DialogContentText component="div">
                  ...
                  <p><strong>August:</strong> <Box component="span" color={selectedRow.August === "No Payment" ? "error.main" : "success.main"}>{selectedRow.August}</Box></p>
